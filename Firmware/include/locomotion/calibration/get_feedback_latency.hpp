...
struct FeedbackLatencyParams
{
    ...
    uint16_t base_delay_ms = 200;
    /// @brief Wait delay between each latency test
    uint16_t test_delay_ms = 200;
};
 ...
 * @note Returned latency value is the average of all latency tests
 */
Error get_feedback_latency(FeedbackLatencyParams params, MotorDriver::Channel motor_channel, AnalogDriver::Channel analog_channel, int& out_value)
{
    ...
    for (int i = 0; i < params.nb_samples; i++)
    {
        ...
        RETURN_ERROR(AnalogDriver::GetVoltage(analog_channel, base_feedback));
        ...
        do
        {
            ...
            RETURN_ERROR(AnalogDriver::ReadAllChannels());
            RETURN_ERROR(AnalogDriver::GetVoltage(analog_channel, new_feedback));
    ...
    // Calculate average latency and return it
    int latency_acc = 0;
    for (int i = 0; i < params.nb_samples; i++) latency_acc += latencies[i];
    out_value = latency_acc / params.nb_samples;
    return Error::None;
}